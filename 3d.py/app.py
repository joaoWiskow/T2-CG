# OpenGLBasico3D-V5-50x50-embeddedmap.py
# Labirinto 50x50 com mapa embutido (mais caminhos) + opção de gerar proceduralmente
...
MAP_SIZE = 50        # 50 x 50
# Se USE_EMBEDDED = True, usa o mapa embutido; se False, gera proceduralmente
USE_EMBEDDED = True
WALL_PROB = 0.06
NUM_CORRIDORS = 120
...
# -------------------- mapa embutido (pronto) - 50x50, MAIS CAMINHOS
def create_embedded_map_50():
    size = 50
    ...
    # "avenidas" internas bem furadas (mais caminho)
    vertical_cols   = [8, 16, 24, 32, 40]
    horizontal_rows = [8, 16, 24, 32, 40]

    # todas as células internas começam vazias (0), vamos desenhar paredes só nesses eixos
    for x in vertical_cols:
        for z in range(2, size-2):
            # buracos frequentes pra ter bastante passagem
            if (z % 7) != 0 and (z % 11) != 0:
                mat[z][x] = CELL_WALL_V
    ...
    for z in horizontal_rows:
        for x in range(2, size-2):
            if (x % 7) != 0 and (x % 11) != 0:
                # se já tem parede vertical, mantemos uma delas; visualmente não muda muito
                if mat[z][x] == 0:
                    mat[z][x] = CELL_WALL_H

    # janelas em alguns pontos estratégicos
    win_positions = [
        (4,4), (4, size-5),
        (size-5,4), (size-5,size-5),
        (size//2,4), (size//2,size-5),
    ]
    for (z,x) in win_positions:
        if 0 <= z < size and 0 <= x < size:
            mat[z][x] = 'J'

    # objetos fixos em áreas abertas
    fixed_positions = [
        (6,6),(6,10),(10,6),
        (12,18),(14,12),
        (20,18),(22,22),
    ...
    # jogador no centro (garantido livre)
    c = size//2
    mat[c][c] = 'P'

    return mat

# -------------------- geração procedural (caso queira alternar)
...
# -------------------- setup do mapa (usa embutido por padrão)
def generate_and_setup_map():
    global Cidade, QtdX, QtdZ
    if USE_EMBEDDED:
        Cidade = create_embedded_map_50()
    else:
...
def is_cell_walkable(x,z):
    if x < 0 or z < 0 or x >= QtdX or z >= QtdZ:
        return False
    v = Cidade[z][x]
    if v == 0 or v == CELL_PLAYER or v == 'P' or v == CELL_FIXED or v == 'F':
        # permitimos caminhar sobre 'F' só para spawn / pathfinding simples
        return True
    return False
...
def main():
    ...
    title = "Labirinto - 50x50 (embedded map, mais caminhos)"
